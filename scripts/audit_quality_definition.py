...
def audit(config: dict[str, Any]) -> dict[str, Any]:
    ...
    result = {
        "markers": config["markers"], "status": "Q0_LOSS_SCALE_AND_BIAS_AUDITED", **environment_metadata(),
    ...
    write_json(output / "q0_loss_scale_and_bias_audit.json", result)
    return result
...
def main() -> None:
    ...
    result = audit(config)
    print(json.dumps({"status": result["status"], "loss_scale": result["loss_scale_audit"]}, ensure_ascii=False))
