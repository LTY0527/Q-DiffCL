...
def _scores_from_runs(normal_runs: np.ndarray, fault_runs: np.ndarray, early_runs: np.ndarray,
                      weights: dict[str, float]) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
...
def build_criticality(features: np.ndarray, bundle: dict[str, np.ndarray], stages: np.ndarray,
    ...
    discriminative, early, stability, composite = _scores_from_runs(
        normal_runs, fault_runs, early_runs, settings)
    ...
    masks = {"energy": _top_mask(energy, ratio), "fisher": _top_mask(discriminative, ratio),
             "composite": _top_mask(composite, ratio)}
    ...
    type_run_means = []; type_labels = []
    for uid in fault_ids:
        type_run_means.append(features[run_uids == uid].mean(0)); type_labels.append(fault_type(str(uid)))
    type_run_means = np.stack(type_run_means); type_labels = np.asarray(type_labels)
    ...
    multiclass_fisher = between / (within + 1e-8)
    ...
    for _ in range(int(settings["bootstrap_repeats"])):
        ...
        _, _, _, boot_composite = _scores_from_runs(sampled_normal, sampled_fault, sampled_early, settings)
